...
const NavButton: React.FC<ButtonProps> = ({ text, classNames, ...props }) => {
  return (
    <button
      {...props}
      className={`${classNames} group relative bg-white text-primary text-sm font-bold rounded-md px-3 py-1 box-shadow-1 border border-transparent hover:border-x hover:border-t hover:border-secondary-2 overflow-clip`}
    >
